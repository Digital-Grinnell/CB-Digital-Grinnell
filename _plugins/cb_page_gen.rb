...
module CollectionBuilderPageGenerator
  class ItemPageGenerator < Jekyll::Generator
    ...
    def generate(site)
      ...
      # get optional configuration from _config.yml, or create a single default one from CB metadata
      ...
      configure_gen.each do |data_config|
        ...
        if filter
          filtered_records = records.select { |record| record[filter] }
        ...
        names_test = records.map { |record| record[name] }
        ...
        template_test = records.map { |record| record[display_template] ? template_location + record[display_template].strip : template }.uniq
